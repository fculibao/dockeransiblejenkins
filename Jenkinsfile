pipeline{
    ...
    stages{
        ...
        stage('Docker Deploy'){
            steps{
              ansiblePlaybook credentialsId: 'ec2-key', disableHostKeyChecking: true, extras: "-e DOCKER_TAG=${DOCKER_TAG}", installation: 'ansible', inventory: 'dev.inv', playbook: 'deploy-docker.yml'
            }
        }
    }
